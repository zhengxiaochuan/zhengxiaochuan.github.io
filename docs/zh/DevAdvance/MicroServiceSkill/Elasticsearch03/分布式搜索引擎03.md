...
聚合的结果与查询结果不同，API也比较特殊。不过同样是JSON逐层解析：
...
/**
  ...
  *
  * @param params
  * @return 聚合结果，格式: {"城市":"上海","北京"], "品牌":["如家","秀尔频]}
  */
...
当用户在搜索框输入字符时，我们应该提示出与该字符有关的搜索项，如下图：
...
      "name": {
        "type": "text",
        "analyzer": "my_analyzer",//创建索引库时使用自定义分词器
        "search_analyzer": "ik_smart" //查询时不需要使用自定义分词器，因为我们只需要根据原样查询即可
      }
    }
  }
}
...
PUT /hotel
{
  "settings": {
    "analysis": {
      "analyzer": {
        "text_anlyzer": {//全文检索字段使用该自定义分词器，先分词再拼音
          "tokenizer": "ik_max_word",
          "filter": "py"
        },
        "completion_analyzer": {//自动补全时使用该自定义分词器，不分词，然后拼音
          "tokenizer": "keyword",
          "filter": "py"
        }
      },
...
// 4.1 根据补全查询名称，获取补全结果
CompletionSuggestion suggestions = suggest.getSuggestion("hotelSuggestion");
// 4.2 获取options并遍历
...
同时在application.yml文件中进行消息队列的配置
